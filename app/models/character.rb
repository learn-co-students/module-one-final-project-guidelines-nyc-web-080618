...
#get input for available targets
      ...
      puts "Select Your Target Number".bold.yellow
      input = gets.chomp
...
#method for the actual combat
  def attack(target)
    ...
    if target == nil
      ...
      puts Artii::Base.new.asciify "*** BATTLE ***"
      puts Artii::Base.new.asciify "*** COMBAT ***"
    ...
    ###and damage to the opponent or current user#####
      ...
      if self.power == target.power
        ...
        if rand > 0.5
          ...
          puts "$$$$$$$$$$$$$$$$$$$$$$$$$$$".blink
          puts "#{self.name} Hash.new { |hash, key| hash[key] =  }as Defeated #{target.name}"
